...
EXP="grpo_qwen3_1p7b_taskenv_think_4gpu_2048_len_$(date +%Y%m%d_%H%M%S)"
RUN_DIR="training/$EXP"

CUDA_DEVICES="${CUDA_DEVICES:-0,1,2,3}"
ROLLOUT_GPU_UTILIZATION="${ROLLOUT_GPU_UTILIZATION:-0.5}"
ROLLOUT_MAX_NUM_SEQS="${ROLLOUT_MAX_NUM_SEQS:-64}"
ROLLOUT_MAX_NUM_BATCHED_TOKENS="${ROLLOUT_MAX_NUM_BATCHED_TOKENS:-4096}"
LENGTH_ALPHA="${LENGTH_ALPHA:-0.03}"
LENGTH_REF_CHARS="${LENGTH_REF_CHARS:-8000}"
APPLY_LEN_ONLY_ON_TRAIN="${APPLY_LEN_ONLY_ON_TRAIN:-true}"
...
reward_path="training/scripts/reward_taskenv_len_wrapper.py"
...
python3 -m verl.trainer.main_ppo \\
  ...
  +reward.custom_reward_function.reward_kwargs.length_alpha=$LENGTH_ALPHA \\
  +reward.custom_reward_function.reward_kwargs.length_ref_chars=$LENGTH_REF_CHARS \\
  +reward.custom_reward_function.reward_kwargs.apply_len_only_on_train=$APPLY_LEN_ONLY_ON_TRAIN \\
...
echo "LENGTH_ALPHA=$LENGTH_ALPHA"
echo "LENGTH_REF_CHARS=$LENGTH_REF_CHARS"
echo "APPLY_LEN_ONLY_ON_TRAIN=$APPLY_LEN_ONLY_ON_TRAIN"
